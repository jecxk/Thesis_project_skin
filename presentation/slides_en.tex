...
\begin{frame}{Data and Challenges}
    \framesubtitle{ISIC 2018 Task 3 (HAM10000) --- 10,015 dermoscopy images, 7 classes, stratified 70/15/15 split}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \scriptsize
        \begin{table}[]
...
\begin{frame}{Appendix --- Contents}
    ...
    \begin{enumerate}
        ...
        \item Confusion matrix --- compact backbones (EB0 vs.\ DN121)
        \item Confusion matrix --- larger backbones (ResNet50 vs.\ Swin-Tiny)
        \item Grad-CAM in action --- from image to heatmap
        \item Ablation confusion --- removing the biggest levers
        \item Ablation confusion --- removing the smaller levers
    \end{enumerate}
    \end{multicols}
\end{frame}
...
% --- A26 ---
\begin{frame}{A26. Confusion Matrix --- Compact Backbones (EB0 vs.\ DN121)}
    \begin{columns}[T]
        \column{0.5\textwidth}
    ...
    \end{columns}
    \vspace{0.2cm}
    \scriptsize \textit{EB0 is the strongest single model (F1 0.831). DN121 leans on feature reuse and matches EB0 on MEL despite being weaker overall. Both CNNs share the same failure mode: confusion inside the \textbf{MEL $\leftrightarrow$ NV $\leftrightarrow$ BKL} cluster.}
\end{frame}

% --- A27 ---
\begin{frame}{A27. Confusion Matrix --- Larger Backbones (ResNet50 vs.\ Swin-Tiny)}
    \begin{columns}[T]
        \column{0.5\textwidth}
    ...
    \end{columns}
    \vspace{0.2cm}
    \scriptsize \textit{ResNet50 is the classical deep CNN baseline. Swin-Tiny is the only Transformer --- it overfits earliest yet ranks probabilities best (highest AUC). Each backbone fails differently, and those uncorrelated errors are exactly what the soft-voting ensemble exploits.}
\end{frame}

% --- A28 ---
\begin{frame}{A28. Grad-CAM in Action --- From Image to Heatmap}
    \footnotesize
...
% --- A29 ---
\begin{frame}{A29. Ablation Confusion --- Removing the Biggest Levers}
    \begin{columns}[T]
        \column{0.5\textwidth}
    ...
    \end{columns}
    \vspace{0.2cm}
    \scriptsize \textit{Without Mixup/CutMix, DF and VASC recall collapses --- the model memorises the 81 DF training images instead of learning generic features. Without Label Smoothing the model becomes over-confident: MEL and BKL start to split off into hard, wrong decisions inside the MEL/NV/BKL cluster.}
\end{frame}

% --- A30 ---
\begin{frame}{A30. Ablation Confusion --- Removing the Smaller Levers}
    \begin{columns}[T]
        \column{0.5\textwidth}
    ...
    \end{columns}
    \vspace{0.2cm}
    \scriptsize \textit{Removing the Weighted Sampler starves rare classes at the batch level --- DF, VASC and AKIEC recall drop noticeably. Removing advanced augmentation (colour jitter, blur, erasing) slightly raises Accuracy on NV but hurts every minority class --- the textbook trade-off that motivated Macro F1 as the primary metric.}
\end{frame}
