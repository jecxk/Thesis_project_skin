...
\begin{frame}{Conclusion}
    \begin{block}{Six main contributions of the thesis}
        \footnotesize
        \begin{enumerate}
            \item \textbf{Image-only ensemble:} \textbf{89.49\%} Accuracy, \textbf{0.980} AUC, \textbf{0.744} F1 for Melanoma at the headline seed; \textbf{89.93 $\pm$ 0.66\%} across 5 seeds.
            \item \textbf{Statistical robustness:} McNemar test confirms ensemble beats every backbone with $p < 10^{-7}$ vs.\ the strongest, $p < 10^{-38}$ vs.\ the weakest.
            \item \textbf{Metadata fusion crosses 90\%:} adding HAM10000 patient metadata yields \textbf{90.49\% / 0.873 F1} --- beats Pacheco \& Krohling 2020.
            \item \textbf{Honest OOD evaluation:} zero-shot on PAD-UFES-20 (smartphone photos) gives 32.5\% --- quantifies the modality gap.
            \item \textbf{Quantitative interpretability:} Grad-CAM \texttt{focus\_ratio} > 0.6 across all backbones, validated by the \texttt{model.bn2} debug story.
            \item \textbf{Practical application:} a stable Streamlit prototype with subprocess-isolated Grad-CAM.
        \end{enumerate}
    \end{block}
...
\begin{frame}{Future Work}
    \footnotesize
    \begin{enumerate}
        \item \textbf{Domain adaptation for clinical photographs:} OOD evaluation on PAD-UFES-20 (appendix A23) shows a $-56$ pp accuracy drop on smartphone images. Closing this gap requires fine-tuning on in-domain data or style-transfer preprocessing.
        \vspace{0.15cm}
        \item \textbf{Open-set / OOD detection:} the current classifier is forced to assign one of 7 classes. A production system needs to recognise inputs that are not skin lesions at all (acne, scars, non-lesion photos).
        \vspace{0.15cm}
