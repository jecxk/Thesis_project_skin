...
\begin{frame}{Introduction}
    ...
    \begin{block}{The clinical problem}
        \small
        Melanoma is the deadliest form of skin cancer --- yet when it is caught early, more than \textbf{99\%} of patients live past five years. Dermoscopy lets doctors look at a lesion up close without any incision, but the final call still comes down to how experienced the dermatologist is.
    \end{block}
    \vspace{0.3cm}
...
\begin{frame}{Introduction}
    \framesubtitle{Thesis Objectives}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Primary goal}
            \footnotesize
            Build an automated system that recognises \textbf{7 types of skin lesions} from dermoscopy images --- \textbf{accurate enough} to be useful, and \textbf{transparent enough} for doctors to trust.
        \end{block}

        \column{0.5\textwidth}
        \textbf{Four concrete contributions}
        \begin{enumerate}\footnotesize
            \item A training pipeline that handles imbalanced data (Weighted Sampler, Mixup, CutMix, Label Smoothing).
            \item A fair comparison of \textbf{four architectures} (three CNNs and one Transformer) under identical conditions.
            \item A \textbf{soft-voting ensemble} of the four models that beats every single one.
            \item \textbf{Grad-CAM} and a Streamlit web app so doctors can see where the model is ``looking''.
        \end{enumerate}
    \end{columns}
\end{frame}
...
\begin{frame}{Data and Challenges}
    \framesubtitle{The ISIC 2018 Task 3 Dataset}
    \begin{columns}[T]
...
\begin{frame}{Data and Challenges}
    \framesubtitle{Consequences of Data Imbalance}
    \begin{columns}[T]
        \column{0.55\textwidth}
        \footnotesize
...
\begin{frame}{Methodology}
    \framesubtitle{Per-Model Hyperparameters}
    \scriptsize
    \begin{table}[]
        \centering
        \begin{tabular}{lcccc}
...
\begin{frame}{Experimental Results}
    \framesubtitle{Single-Model Performance}
    \scriptsize
    \begin{table}[]
        \centering
...
\begin{frame}{Experimental Results}
    \framesubtitle{Ensemble: Combining Four Models via Soft Voting}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \footnotesize
...
\begin{frame}{Experimental Results}
    \framesubtitle{Per-Class Analysis (EB0 vs.\ Ensemble)}
    \scriptsize
    \begin{table}[]
        \centering
...
\begin{frame}{Ablation Study}
    \framesubtitle{Interpreting the Ablation Results}
    \footnotesize
...
\begin{frame}{Interpretability and Application}
    \framesubtitle{Grad-CAM --- Looking at the Model's "Reasons"}
    \begin{columns}[T]
        \column{0.45\textwidth}
        \footnotesize
...
\begin{frame}{Interpretability and Application}
    \framesubtitle{Streamlit Prototype --- A Real-Time "Second Opinion"}
    \begin{columns}[T]
        \column{0.55\textwidth}
        \footnotesize
        \textbf{Key features:}
...
\begin{frame}{Conclusion}
    \framesubtitle{Six Main Contributions}
