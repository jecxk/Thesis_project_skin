...
\begin{frame}{Introduction}
    ...
    \begin{columns}[T]
        ...
        \begin{enumerate}\footnotesize
            ...
            \item Grad-CAM visual explanations for clinical trust.
        \end{enumerate}
    \end{columns}
\end{frame}
...
\begin{frame}{Data and Challenges}
    ...
    \begin{columns}[T]
        ...
        \vspace{0.1cm}
        \centering
        \textcolor{usthred}{\footnotesize Extreme imbalance: \textbf{NV : DF $\approx$ 58 : 1}} \\[3pt]
        \footnotesize Stratified \textbf{70/15/15} split --- train/val/test keep the same class ratios.
...
\begin{frame}{Methodology}
    \framesubtitle{Data Preprocessing and Training Augmentation}
    \footnotesize
    \begin{columns}[T]
        \column{0.36\textwidth}
        \begin{block}{Preprocessing (all splits)}
            \footnotesize
            \begin{itemize}
                \item Resize to $224 \times 224$, normalize with ImageNet mean/std.
            \end{itemize}
        \end{block}
        \vspace{0.15cm}
        \begin{block}{Augmentation}
            \footnotesize
            \begin{itemize}
                \item RandomResizedCrop, horizontal $+$ vertical flip, rotation ($\pm 90^\circ$).
...
\begin{frame}{Methodology}
    ...
    \begin{table}[]
        ...
        \textbf{Model} & \textbf{Params} & \textbf{Role in the ensemble} \\
        \midrule
        EfficientNet-B0 & 5.3M  & Compact CNN, fine local features \\
        ResNet50        & 25.6M & Deep CNN, classical baseline     \\
        DenseNet121     & 8.0M  & CNN exploiting feature reuse     \\
        Swin-Tiny       & 28.0M & Vision Transformer, global context \\
        \bottomrule
        \end{tabular}
    \end{table}
    \begin{figure}
        ...
        \caption*{\scriptsize Swin-Tiny stops earliest (epoch 13/25) --- an early sign of overfitting.}
    \end{figure}
\end{frame}
...
\begin{frame}{Interpretability and Application}
    \framesubtitle{Grad-CAM --- Visual Explanation}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{figure}
            \centering
        ...
        \vspace{0.15cm}
        \begin{block}{Quantitative validation}
            \scriptsize
            Beyond visual inspection: on the test set, more than \textbf{60\% of each heatmap's ``hottest'' area} falls inside the actual lesion --- for all four models, not on surrounding skin.
        \end{block}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Key contributions}
            ...
            \begin{enumerate}
                ...
                \item Grad-CAM heatmaps stay focused on the lesion --- over 60\% of the ``hot'' area lands inside it, not on artefacts.
            \end{enumerate}
        \end{block}
...
\begin{frame}[plain]
    ...
    {\Large \textcolor{usthdark}{Questions and Discussion}}\\[0.6cm]
    \begin{tikzpicture}
...
\begin{frame}{Appendix --- Contents}
    ...
    \begin{enumerate}
        ...
        \item Training strategy --- Mixup/CutMix, Label Smoothing, Weighted Sampler, optimisation
        \item Computing Robustness and McNemar --- worked example
    \end{enumerate}
    \end{multicols}
\end{frame}
...
% --- A28 ---
\begin{frame}{A28. Training Strategy}
    \begin{columns}[T]
        \column{0.5\textwidth}
...
% --- A29 ---
\begin{frame}{A29. Computing Robustness and McNemar (worked example)}
    \scriptsize
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Robustness across 5 seeds --- Mean/Std}
            \scriptsize
            5 real Ensemble accuracy values: 88.89, 89.82, 90.69, 90.09, 90.15\%\\[3pt]
            \textbf{Mean:} $(88.89{+}89.82{+}90.69{+}90.09{+}90.15)/5 = 89.93\%$\\[3pt]
            \textbf{Std (sample, divide by $n{-}1{=}4$):}\\
            Deviations: $-1.04, -0.11, +0.76, +0.16, +0.22$\\
            $\sum (\text{deviation})^2 = 1.746$\\
            $\mathrm{Std} = \sqrt{1.746/4} \approx 0.66\%$
        \end{block}

        \column{0.5\textwidth}
        \begin{block}{McNemar --- example: seed 42, vs.\ Swin-Tiny}
            \scriptsize
            $n_{ab}$ (Ens.\ right, Swin wrong) $=33$\\
            $n_{ba}$ (Ens.\ wrong, Swin right) $=93$\\[3pt]
    ...
    \vspace{0.2cm}
    \begin{block}{Combining 5 seeds with Fisher's method}
        \scriptsize
        $\chi^2_{\text{combined}} = -2\sum_{i=1}^{5}\ln(p_i) = -2(-25.90) = 51.80$, $df=2{\times}5=10$\\
        $p_{\text{final}} = \chi^2_{\text{sf}}(51.80, df{=}10) \approx 1.24{\times}10^{-7}$ --- matches the figure on A21.\\
        \textit{Note: only 3/5 seeds are individually significant at $p<0.05$, yet the ensemble wins on 5/5 seeds --- Fisher's method pools that consistent evidence into one very confident conclusion.}
    \end{block}
\end{frame}
