...
\begin{frame}{Giới thiệu}
    \framesubtitle{Bối cảnh và động lực}
    \begin{block}{Vấn đề lâm sàng}
        \small
        Ung thư hắc tố (melanoma) là dạng ung thư da nguy hiểm nhất --- nhưng nếu phát hiện sớm, hơn \textbf{99\%} bệnh nhân sống khỏe sau 5 năm. Kính soi da (dermoscopy) giúp bác sĩ nhìn rõ tổn thương mà không cần chích rạch, tuy nhiên kết luận cuối cùng vẫn phụ thuộc nhiều vào kinh nghiệm người khám.
    \end{block}
    \vspace{0.3cm}
...
\begin{frame}{Giới thiệu}
    \framesubtitle{Mục tiêu khóa luận}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Mục tiêu chính}
            \footnotesize
            Xây dựng một hệ thống tự động nhận biết \textbf{7 loại tổn thương da} từ ảnh soi da, vừa \textbf{đủ chính xác} vừa \textbf{giải thích được} để bác sĩ có thể tin dùng.
        \end{block}

        \column{0.5\textwidth}
        \textbf{Bốn đóng góp cụ thể}
        \begin{enumerate}\footnotesize
            \item Pipeline huấn luyện chống lệch dữ liệu (Weighted Sampler, Mixup, CutMix, Label Smoothing).
            \item So sánh công bằng \textbf{4 kiến trúc} (3 CNN $+$ 1 Transformer) trên cùng điều kiện.
            \item \textbf{Kết hợp 4 mô hình theo soft-voting}, vượt mọi mô hình đơn lẻ.
            \item \textbf{Grad-CAM} và ứng dụng web Streamlit để bác sĩ thấy mô hình đang ``nhìn'' vào đâu.
        \end{enumerate}
    \end{columns}
\end{frame}
...
\begin{frame}{Dữ liệu và thách thức}
    \framesubtitle{Bộ dữ liệu ISIC 2018 Task 3}
    \begin{columns}[T]
...
\begin{frame}{Dữ liệu và thách thức}
    \framesubtitle{Hệ quả của mất cân bằng dữ liệu}
    \begin{columns}[T]
        \column{0.55\textwidth}
        \footnotesize
...
\begin{frame}{Phương pháp}
    \framesubtitle{Siêu tham số theo mô hình}
    \scriptsize
    \begin{table}[]
        \centering
        \begin{tabular}{lcccc}
...
\begin{frame}{Kết quả thực nghiệm}
    \framesubtitle{Hiệu năng từng mô hình đơn}
    \scriptsize
    \begin{table}[]
        \centering
...
\begin{frame}{Kết quả thực nghiệm}
    \framesubtitle{Ensemble: kết hợp 4 mô hình bằng soft voting}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \footnotesize
        ...
        \begin{block}{Kết quả Ensemble (test set 1\,503 ảnh)}
            ...
            1 hạt giống: Acc \textbf{89.49\%} \quad F1 \textbf{0.845} \quad AUC \textbf{0.980}\\[2pt]
            5 hạt giống: \textbf{$89.93 \pm 0.66\%$} \quad \textbf{$0.852 \pm 0.010$} \quad \textbf{$0.982 \pm 0.001$}
        \end{block}
...
\begin{frame}{Kết quả thực nghiệm}
    \framesubtitle{Phân tích chi tiết theo lớp (EB0 vs.\ Ensemble)}
    \scriptsize
    \begin{table}[]
        \centering
...
\begin{frame}{Nghiên cứu loại bỏ thành phần}
    \framesubtitle{Diễn giải kết quả Ablation}
    \footnotesize
...
\begin{frame}{Khả diễn giải và ứng dụng}
    \framesubtitle{Grad-CAM --- nhìn vào "lý do" mô hình quyết định}
    \begin{columns}[T]
        \column{0.45\textwidth}
        \footnotesize
...
\begin{frame}{Khả diễn giải và ứng dụng}
    \framesubtitle{Prototype Streamlit --- "second opinion" thời gian thực}
    \begin{columns}[T]
        \column{0.55\textwidth}
        \footnotesize
...
\begin{frame}{Kết luận}
    \framesubtitle{Sáu đóng góp chính}
    \begin{block}{Sáu đóng góp chính của khóa luận}
        \footnotesize
        \begin{enumerate}
            \item \textbf{Ensemble chỉ-ảnh:} \textbf{89.49\%} Acc, \textbf{0.980} AUC, \textbf{0.744} F1 cho Melanoma ở hạt giống đại diện; \textbf{89.93 $\pm$ 0.66\%} qua 5 hạt giống.
            \item \textbf{Robustness thống kê:} McNemar xác nhận ensemble vượt mọi backbone với $p < 10^{-7}$ so với mạnh nhất, $p < 10^{-38}$ so với yếu nhất.
            \item \textbf{Metadata fusion vượt 90\%:} thêm metadata bệnh nhân HAM10000 đạt \textbf{90.49\% / 0.873 F1} --- vượt Pacheco \& Krohling 2020.
            \item \textbf{Đánh giá OOD trung thực:} zero-shot trên PAD-UFES-20 (ảnh smartphone) đạt 32.5\% --- định lượng khoảng cách modality.
            \item \textbf{Khả diễn giải định lượng:} Grad-CAM \texttt{focus\_ratio} > 0.6 trên cả 4 backbone, được validate qua story debug \texttt{model.bn2}.
            \item \textbf{Ứng dụng thực tế:} prototype Streamlit ổn định với subprocess-isolated Grad-CAM.
        \end{enumerate}
    \end{block}
...
\begin{frame}{Kết luận}
    \framesubtitle{Hướng phát triển}
    \footnotesize
    \begin{enumerate}
        \item \textbf{Domain adaptation cho ảnh smartphone:} kết quả OOD trên PAD-UFES-20 (phụ lục A23) cho thấy drop $-56$ pp accuracy. Đóng khoảng cách cần fine-tune trên data in-domain hoặc style-transfer preprocessing.
        \vspace{0.15cm}
        \item \textbf{Open-set / OOD detection:} classifier hiện tại bị buộc gán 1 trong 7 lớp. Production cần component nhận diện input \textit{không phải} tổn thương da (mụn, sẹo, ảnh sai vùng).
        \vspace{0.15cm}
