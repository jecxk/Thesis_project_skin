...
\begin{frame}{Giới thiệu}
    ...
    \begin{columns}[T]
        ...
        \begin{enumerate}\footnotesize
            ...
            \item Giải thích trực quan bằng Grad-CAM để bác sĩ tin cậy.
        \end{enumerate}
    \end{columns}
\end{frame}
...
\begin{frame}{Dữ liệu và thách thức}
    ...
    \begin{columns}[T]
        ...
        \vspace{0.1cm}
        \centering
        \textcolor{usthred}{\footnotesize Mất cân bằng cực đoan: \textbf{NV : DF $\approx$ 58 : 1}} \\[3pt]
        \footnotesize Chia stratified \textbf{70/15/15} --- train/val/test giữ đúng tỷ lệ các lớp.
...
\begin{frame}{Phương pháp}
    ...
    \vspace{0.1cm}
    \begin{center}
        \includegraphics[width=0.97\textwidth,height=0.85\textheight,keepaspectratio]{../thesis/figures/pipeline_overview.png}
    \end{center}
    \vspace{0.15cm}
\end{frame}

\begin{frame}{Phương pháp}
    \framesubtitle{Tiền xử lý Dữ liệu và Tăng cường khi Huấn luyện}
    \footnotesize
    \begin{columns}[T]
        \column{0.36\textwidth}
        \begin{block}{Tiền xử lý (mọi tập)}
            \footnotesize
            \begin{itemize}
                \item Resize về $224 \times 224$, chuẩn hóa theo mean/std ImageNet.
            \end{itemize}
        \end{block}
        \vspace{0.15cm}
        \begin{block}{Tăng cường dữ liệu}
            \footnotesize
            \begin{itemize}
                \item RandomResizedCrop, lật ngang $+$ dọc, xoay ($\pm 90^\circ$).
                \item ColorJitter ($0.25$), Gaussian Blur ($p=0.1$), Random Erasing ($p=0.1$).
            \end{itemize}
        \end{block}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=\linewidth,height=5.8cm,keepaspectratio]{../thesis/figures/augmentation_examples.png}
        \end{figure}
    \end{columns}
\end{frame}
...
\begin{frame}{Phương pháp}
    ...
    \begin{table}[]
        ...
        \textbf{Mô hình} & \textbf{Tham số} & \textbf{Vai trò trong ensemble} \\
        \midrule
        EfficientNet-B0 & 5.3M  & CNN gọn, đặc trưng local mịn  \\
        ResNet50        & 25.6M & CNN sâu, baseline cổ điển     \\
        DenseNet121     & 8.0M  & CNN tận dụng feature reuse    \\
        Swin-Tiny       & 28.0M & Vision Transformer, ngữ cảnh toàn cục \\
        \bottomrule
        \end{tabular}
    \end{table}
    \begin{figure}
        \centering
        \includegraphics[width=0.65\textwidth,keepaspectratio]{../thesis/figures_new/training_loss_all_models.png}
        \caption*{\scriptsize Swin-Tiny dừng sớm nhất (epoch 13/25) --- dấu hiệu overfit sớm.}
    \end{figure}
\end{frame}
...
\begin{frame}{Khả diễn giải và ứng dụng}
    \framesubtitle{Grad-CAM --- Giải thích trực quan}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{figure}
            \centering
        ...
        \vspace{0.15cm}
        \begin{block}{Kiểm chứng định lượng}
            \scriptsize
            Không chỉ đánh giá bằng mắt: trên tập test, hơn \textbf{60\% vùng ``nóng nhất''} của heatmap nằm đúng trong tổn thương --- ở cả 4 mô hình, không lan ra da xung quanh.
        \end{block}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Đóng góp chính}
            ...
            \begin{enumerate}
                ...
                \item Heatmap Grad-CAM tập trung đúng chỗ --- hơn 60\% vùng ``nóng'' nằm trong tổn thương, không vào nhiễu.
            \end{enumerate}
        \end{block}
...
\begin{frame}[plain]
    ...
    {\Large \textcolor{usthdark}{Câu hỏi và Thảo luận}}\\[0.6cm]
    \begin{tikzpicture}
...
\begin{frame}{Phụ lục --- Mục lục}
    ...
    \begin{enumerate}
        ...
        \item So sánh ma trận nhầm lẫn giữa 4 backbone
        \item Grad-CAM hoạt động ra sao --- từ ảnh đến heatmap
        \item Chiến lược huấn luyện --- Mixup/CutMix, Label Smoothing, Weighted Sampler, tối ưu hóa
        \item Cách tính Robustness và McNemar --- ví dụ số thật
    \end{enumerate}
    \end{multicols}
\end{frame}
...
% --- A26 ---
\begin{frame}{A26. So sánh ma trận nhầm lẫn --- Bốn backbone}
    \scriptsize
    \begin{columns}[T]
        \column{0.5\textwidth}
...
% --- A27 ---
\begin{frame}{A27. Grad-CAM hoạt động ra sao --- Từ ảnh đến heatmap}
    \footnotesize
...
% --- A28 ---
\begin{frame}{A28. Chiến lược huấn luyện}
    \begin{columns}[T]
        \column{0.5\textwidth}
...
% --- A29 ---
\begin{frame}{A29. Cách tính Robustness và McNemar (ví dụ số thật)}
    \scriptsize
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Robustness qua 5 seed --- Mean/Std}
            \scriptsize
            5 giá trị Accuracy Ensemble thật: 88.89, 89.82, 90.69, 90.09, 90.15\%\\[3pt]
            \textbf{Mean:} $(88.89{+}89.82{+}90.69{+}90.09{+}90.15)/5 = 89.93\%$\\[3pt]
            \textbf{Std (mẫu, chia $n{-}1{=}4$):}\\
            Độ lệch: $-1.04, -0.11, +0.76, +0.16, +0.22$\\
            $\sum (\text{độ lệch})^2 = 1.746$\\
            $\mathrm{Std} = \sqrt{1.746/4} \approx 0.66\%$
        \end{block}

        \column{0.5\textwidth}
        \begin{block}{McNemar --- ví dụ seed 42, vs.\ Swin-Tiny}
            \scriptsize
            $n_{ab}$ (Ens.\ đúng, Swin sai) $=33$\\
            $n_{ba}$ (Ens.\ sai, Swin đúng) $=93$\\[3pt]
            $\text{statistic} = \dfrac{(|n_{ab}-n_{ba}|-1)^2}{n_{ab}+n_{ba}} = \dfrac{59^2}{126} \approx 27.63$\\[3pt]
            $p = \chi^2_{\text{sf}}(27.63, df{=}1) \approx 1.47{\times}10^{-7}$
        \end{block}
    \end{columns}

    \vspace{0.2cm}
    \begin{block}{Gộp 5 seed bằng Fisher's method}
        \scriptsize
        $\chi^2_{\text{combined}} = -2\sum_{i=1}^{5}\ln(p_i) = -2(-25.90) = 51.80$, $df=2{\times}5=10$\\
        $p_{\text{final}} = \chi^2_{\text{sf}}(51.80, df{=}10) \approx 1.24{\times}10^{-7}$ --- khớp đúng số ở A21.\\
        \textit{Lưu ý: chỉ 3/5 seed có $p$ riêng $<0.05$, nhưng ensemble thắng 5/5 seed --- Fisher's method gộp bằng chứng nhất quán đó thành 1 kết luận rất chắc chắn.}
    \end{block}
\end{frame}
